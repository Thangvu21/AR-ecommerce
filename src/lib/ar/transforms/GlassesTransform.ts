...
export class GlassesTransform implements ITransformCalculator {
  private readonly DEFAULT_EYE_DISTANCE_DIVISOR = 400;
  ...
  calculate(
    ...
    const posX = (eyeCenter[0] - videoSize.width / 2) * -0.01 + offsetX + initAdjustPos.x;
    const posY = (eyeCenter[1] - videoSize.height / 2) * -0.01 + offsetY + initAdjustPos.y;
    ...
    const noseDrop = noseTip[2] - eyeCenter[2];
